...
/**
 * Клас для генерації токена й підготовки відповіді для клиента
 */
public class GenerateJwtTokenAndPrepareResponse implements Function<UserLoginRequest, LoginUserResponse> {
    ...
    public LoginUserResponse apply(UserLoginRequest candidate) {
        ...
        TokenInfo tokenInfo = this.getGeneratedJwtTokenInfo(user);
        // Згенерований токен
        String token = tokenInfo.getToken();
        // Зберігаємо токен в базі активних токенів
        this.saveAuthSessionToDatabase(user.getId(), tokenInfo);
     ...
     */
    private TokenInfo getGeneratedJwtTokenInfo(User user) {
        UUID sid = UUID.randomUUID();
        return jwtService.generateToken(user.getId(), user.getEmail(), sid);
    }
    ...
    /**
     * Зберегти активну сесію в базу даних
     *
     * @param userId ID користувача
     * @param tokenInfo інформація про створений токен
     */
    private void saveAuthSessionToDatabase(Long userId, TokenInfo tokenInfo) {
